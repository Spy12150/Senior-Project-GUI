...
class blobfish:
    def __init__(self, depth):
        ...
        self.model = tf.keras.models.load_model('blobfish3.h5')
    ...
    def get_best_move(self, board, color):
        ...
        if color == "w":
            ...
            alpha = -float('inf')
            beta = float('inf')
            initialEval = self.AIevaluate(board, color)
            for move in captures:
                ...
                i += 1
                print(f"\rProgress: {round((i/n)*100, 1)}%", end='')

                if evaluation > max_evaluation and evaluation > initialEval:
            ...
            for move in captures:
                ...
                if evaluation < min_evaluation and evaluation < initialEval:
                    min_evaluation = evaluation
                    best_move = move

                alpha = min(alpha, min_evaluation)
    ...
    def minimax(self, board, depth, color, alpha, beta):
        ...
        if depth == 0 or len(legalmoves) == 0:
            ...
            return self.minimax_captures(board,2, color, alpha, beta)
    ...
    def minimax_captures(self, board, depth, color, alpha, beta):
        board = board
        if color == "w":
            otherplayer = "b"
        else:
        ...
        legalmoves = self.list.get_captures(board, color)

        if len(self.list.get_legal_moves(board,color)) == 0 and self.list.is_king_in_check(board, color):
            if color == "w":
                return -9999999
            else:
                return 9999999
        ...
        if color == "w":
            ...
            for move in legalmoves:
                boardcopy = self.Test_Move(move, copy.deepcopy(board))
                evaluation = self.minimax_captures(boardcopy, depth - 1, "b", alpha, beta)
                max_evaluation = max(max_evaluation, evaluation)
                alpha = max(alpha, max_evaluation)
            ...
            for move in legalmoves:
                boardcopy = self.Test_Move(move, copy.deepcopy(board))
                evaluation = self.minimax_captures(boardcopy, depth - 1, "w", alpha, beta)
                min_evaluation = min(min_evaluation, evaluation)
                beta = min(beta, min_evaluation)
